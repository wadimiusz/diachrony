...
def main():
    ...
    word = random.sample(set(vocabs[0]), 1)[0]
    ...
    for pair in tqdm(pairs):
        ...
        for sample in old_samples:
            sample_vec = avg_feature_vector(sample[0], model=model1, num_features=300)

            sample_dict = {'vec': sample_vec, 'sent': sample[1]}
            old_samples_vec.append(sample_dict)

        for sample in new_samples:
            sample_vec = avg_feature_vector(sample[0], model=model2, num_features=300)
            sample_dict = {'vec': sample_vec, 'sent': sample[1]}
