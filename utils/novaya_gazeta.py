...
def get_issues(startpage: str):
    ...
    if os.path.exists(link_path):
        ...
        issues = [link for link in open(link_path, "r", encoding="utf-8").read().split("\n") if link]
        return issues
    issues = []
    issue_number = int(startpage.split('/')[-1])    # берем ссылку на последний в 2019 году номер архива
...
def get_articles(issues: list):
    ...
    articles = []
    issues = [i for i in issues if i.startswith("http")]
    for issue in issues:
        sleep(randint(1, 3))
