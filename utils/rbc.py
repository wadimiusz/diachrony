...
def crawl_sitemap(sitemaps, n=512):
    ...
    и дат их создания такой, что
    key:value = link:date и len(all_links) == len(set(all_links)) == len(all_dates),
    т.е. ссылки корректно мэтчатся с датами"""
    user_agent = 'Chrome/72.0.3626.119'
    all_links, all_dates = [], []
    if n == 0 or n > 512:
        raise Exception(
            'N should not exceed 512 or be of 0. The entered value of N was: {}.'.format(n))
    for sitemap in get_sitemap(sitemaps)[n:]:
        sitemap = BeautifulSoup(requests.get(sitemap, timeout=100000,
                                             headers={'user-agent': user_agent}).text, 'lxml')
        chunks = re.findall('http.+\s.+T', str(sitemap))
        # http://top.rbc.ru/economics/04/10/2011/618554.shtml</loc>\n<lastmod>2012-02-06T
        text_chunks = [chunk.replace('</loc>\n<lastmod>', '*').replace('T', '') for chunk in chunks
