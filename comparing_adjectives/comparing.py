...
results_eval_regular['std_from_meanvec'] = results_eval_regular['WORD'].map(eval_deviation_regular)
results_eval_incremental['std_from_meanvec'] = results_eval_incremental['WORD'].map(eval_deviation_incremental)
results_rest_regular['std_from_meanvec'] = results_rest_regular['WORD'].map(rest_deviation_regular)
results_rest_incremental['std_from_meanvec'] = results_rest_incremental['WORD'].map(rest_deviation_incremental)
...
move_eval_incr_proc = get_move_from_initial_procrustes(words_incremental, aligned_models_incremental)
move_eval_incr_ga = get_move_from_initial_globalanchors(words_incremental, aligned_models_incremental)
...
move_rest_incr_proc = get_move_from_initial_procrustes(rest_incremental, aligned_models_incremental)
move_rest_incr_ga = get_move_from_initial_globalanchors(rest_incremental, aligned_models_incremental)

results_eval_regular['sum_deltas_procrustes'] = results_eval_regular['WORD'].map(move_eval_reg_proc)
results_eval_regular['sum_deltas_globalanchors'] = results_eval_regular['WORD'].map(move_eval_reg_ga)

results_eval_incremental['sum_deltas_procrustes'] = results_eval_incremental['WORD'].map(move_eval_incr_proc)
results_eval_incremental['sum_deltas_globalanchors'] = results_eval_incremental['WORD'].map(move_eval_incr_ga)

results_rest_regular['sum_deltas_procrustes'] = results_rest_regular['WORD'].map(move_rest_reg_proc)
results_rest_regular['sum_deltas_globalanchors'] = results_rest_regular['WORD'].map(move_rest_reg_ga)

results_rest_incremental['sum_deltas_procrustes'] = results_rest_incremental['WORD'].map(move_rest_incr_proc)
results_rest_incremental['sum_deltas_globalanchors'] = results_rest_incremental['WORD'].map(move_rest_incr_ga)
